...
The `System` service is a simple REST service that returns the properties of a system and
is provided to you.
...
your application, label them both with the `@ApplicationScoped` annotation.
...
To build the application, run the following command from the `start` directory:
```
mvn install
```

This command builds the application and creates a .war file in the `target` directory under both the `rest/` and
...
This command rebuilds the application, and the Liberty server that is running automatically picks up the changes.
...
run the `mvn clean install` command from the command line. It might take some time before
the tests are executed. You see a similar output if the tests passed successfully:
