...
The fastest way to work through this guide is to clone the git repository and use the `start` directory
...
Feel free to try the finished version of what you will be building in this guide to get a feel of
how everything will work. To do this, navigate to the `finish` directory and execute the following:

```
mvn clean install liberty:start-server
```

Your application is now running in Open Liberty at http://localhost:9080/inventory/hosts. Since you
haven't stored anything in the inventory just yet, it will appear empty:
...
Try retrieving your own system properties. After you retrieve them, they will be automatically stored
in the inventory. To do this, point your browser to http://localhost:9080/inventory/hosts/localhost.
You will see a large JSON with your JVM's system properties. Next, return back to http://localhost:9080/inventory/hosts
and you will see them stored under `localhost`:
...
Feel free to try this with your machine's hostname as well. It will add a new entry to the inventory
under `<your hostname>`. You can also run the `system` service on any other local machine and repeat
the same process.
