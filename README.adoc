...
the system properties for a particular host and view a list of all currently stored hosts. If the user
...
Individual host's full system properties will also be accessible by passing the hostname instead:
```
http://localhost:9080/inventory/hosts/<hostname>
```

NOTE: The `system` and the `inventory` services will both be run locally on a single server.
