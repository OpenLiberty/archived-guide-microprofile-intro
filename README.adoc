...
This command builds the application and creates a `.war` file in the target directory. The command also
configures and installs Liberty into the `target/liberty/wlp` directory. After the Maven build completes,
you can use the server script in that Liberty installation. You can also start and stop the server by
using the Maven goals of `liberty:start-server` and `liberty:stop-server`.
