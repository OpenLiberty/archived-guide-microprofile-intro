...
MicroProfile is an initiative to optimize Enterprise Java for a microservices architecture. It began
...
and also use CDI to inject dependencies and help manage the lifecycle of your application.
...
from the `System` service of the specified system. The `System` service is a simple REST service
that returns the properties of a system and is provided to you.

You will implement this application in the local case that is with everything deployed on one system.
...
Notice the calls to `InventoryUtil`. This class contains utility methods that test connections
...
CDI simplifies the management of dependencies and lifecycle and
enables Java EE developers to focus on their code.

=== Injection

To use the dependencies that you need with CDI, use the `@Inject` annotation to easily inject them into your code.

In the previous section, you created an `InventoryManager` bean. Inject that bean into
the `InventoryResource` dependency to use what the bean provides without cumbersome instantiation.
...
In general, you want to make resources as `@ApplicationScoped` unless they depend on request-specific state.
...
Annotate `InventoryManager` with `@ApplicationScoped` too because it does not depend on any request-specifc state either.
If a real database resource is used instead of a `Map`, it improves performance as database resources
are not re-initialized on each request.
...
http://localhost:9081/LibertyProject/Inventory/systems/localhost
```

The first URL returns the current contents of the inventory. The second URL returns the system
properties for the particular hostname. The service retrieves the properties from the inventory.
If the inventory does not have an entry for the particular hostname, the service instead calls
...
If you start the servers, you can access the two previous URLs from your browser to manually test the application.
However, rely on automated tests because these tests trigger a failure if a code change introduces a bug.
...
To make the information easier to change, specify it in a single place like the Maven pom.xml
...
You can then access these properties with the `System.getProperties` method. Use the properties
to create representations of the URL associated with each running service:
...
each test method within a single container method. This method is then the only method labeled `@Test`.
...
particular URL. Reuse the line to keep your code neat and organized.
...
valid and returns status code 200.
...
Create a test called `testHostRegistration`. This test asserts that registering
the `localhost` system is successful.
...
Create a test called `testSystemPropertiesMatch`. This test asserts that the
system properties that are stored in the inventory match the properties that the `System` service returns.
...
To rebuild and run the tests, navigate to the `guide-{projectid}/start` directory and
run the `mvn clean install` command from the command line.

```
# If servers are still running from previous steps, stop them first
cd microprofile/
mvn liberty:stop-server -DserverName=microServer
cd ../rest/
...
It might take some time before the tests are executed.
You see a similar output if the tests passed successfully:
...
Congratulations! You built and tested a MicroProfile application with JAX-RS, CDI, JSON-P
and Open Liberty.
