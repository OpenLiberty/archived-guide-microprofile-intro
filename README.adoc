...
This application provides an `inventory` service that allows you to retrieve the information for a particular
host or a list of all previously registered hosts. If the requested host is not in the inventory, the `inventory`
service attempts to retrieve that host's information by calling its `system` service.
...
Context and Dependency Injection (CDI) is a Java EE feature that makes it simple for developers to
use enterprise beans in web applications.
...
In general, all resourced should be annotated with `@ApplicationScoped` unless they should be created once
per every request.
...
Once the server is running, you can find both services at:
...
You can then access these properties via the `System.getProperties` method. Use these properties
...
particular URL. This helps keep your code neat and organized.
